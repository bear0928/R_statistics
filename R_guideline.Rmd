...
#### B.計算 p 值
使用 pf() 函數來計算與樣本 F 統計量相關的 p 值。由於這是雙尾檢驗，我們需要將計算出的概率乘以 2。
```{r 11-6}
# 計算 p 值
...
library(fBasics)
# 讀取檔案
file_path <- "/Users/bear/dev/R/Statistics/Data Files/Chapter12.xlsx" # 更改成您要讀取之xlsx的路徑
sheet_name_1 <- "Household_Income" #這裡輸入您要讀取的sheet的名稱

# 使用read.xlsx函數讀取指定的工作表
household.income.df <- read.xlsx(file_path, sheet = sheet_name_1)
summary(household.income.df)
```
...
# 讀取檔案
file_path <- "/Users/bear/dev/R/Statistics/Data Files/Chapter14.xlsx" # 更改成您要讀取之xlsx的路徑
sheet_name_1 <- "Debt_Payments" #這裡輸入您要讀取的sheet的名稱

# 使用read.xlsx函數讀取指定的工作表
debt.payments.df <- read.xlsx(file_path, sheet = sheet_name_1)
summary(debt.payments.df)
```
...
```{r 14-2A}
# 構建線性迴歸模型
# 首先，我們使用 lm() 函數來構建一個線性模型對象。這個模型將債務 (Debt) 作為應變量，收入 (Income) 作為自變量。
